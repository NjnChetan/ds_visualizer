...
def visualize(l, left, right, mid=None, compare_index=None, found_index=None, result_message=None):
    ...
    node_width = 80  # Width of each node block
    node_height = 60  # Height of each node block
    ...
    for i in range(num_elements):
        ...
        if found_index is not None and i == found_index:
            ...
            color = yellow  # Use yellow for the search range
        else:
            color = gray  # Gray out elements that are not in the current search range
        ...
        py.draw.rect(pygame_surface, black, (node_x - 5, node_y - 5, node_width + 10, node_height + 10), border_radius=10)  # Border around block
    ...
    # Render the middle element being compared
    if mid is not None:
        mid_text = f"Mid: {l[mid]}"
        mid_render = font.render(mid_text, True, white)
        mid_rect = mid_render.get_rect(center=(w // 2, 50))  # Positioned at the top-center
        pygame_surface.blit(mid_render, mid_rect)
...
def binary_search(l):
    ...
    def search_step():
        ...
        if is_paused:
            app.after(100, search_step)
            return

        if left <= right:
            mid = (left + right) // 2  # Find the middle index

            visualize(l, left, right, mid=mid, compare_index=mid)

            if l[mid] == search_target:
                result_message = f"Found at position {mid + 1}"  # +1 for 1-based indexing
                visualize(l, left, right, found_index=mid, result_message=result_message)
                return  # Search complete
            elif l[mid] < search_target:
                left = mid + 1  # Move to the right half
            else:
                right = mid - 1  # Move to the left half

            app.after(int(animation_speed), search_step)  # Continue search
        else:
            # Target not found
            result_message = "Not Found"
            visualize(l, left, right, result_message=result_message)
...
def start_searching():
    ...
    try:
        ...
        input_list.sort()  # Binary Search requires the list to be sorted
...
app = ctk.CTk()
app.title("Binary Search Visualizer")
...
# Heading
head_label = ctk.CTkLabel(control_frame, text="Binary Search Visualizer", font=("Arial", 20, "bold"))
head_label.grid(row=0, column=0, padx=8, pady=10, columnspan=2)

# Input box
input_label = ctk.CTkLabel(control_frame, text="Enter sorted lis (comma-separated):", font=("Arial", 16))
